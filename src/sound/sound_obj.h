...
namespace Capture
{
	...
	public:
		...
		SoundObj(long sr, short nc, short bps, long ds)
		...
		SoundObj(long sr, short bps, double* mem, long size)
		{
			sampleRate = sr;
			numChannels = 1;
			bitsPerSample = bps;
		...
		SoundObj& operator=(const SoundObj& rhs)
		{
			if (this == &rhs)
				return *this;
		...
		SoundObj CreateMono16Bit(int ch)
		{
			if ((numChannels == 1 && ch == 1) || bitsPerSample == 8)
				throw (0);
			SoundObj result(sampleRate, 1, 16, dataSize / numChannels);
			GetAllSamples16Bit(ch, reinterpret_cast<short*>(result.GetPtrData()));
			return result;
		}

		SoundObj CreateMono8Bit(int ch)
		{
			if ((numChannels == 1 && ch == 1) || bitsPerSample == 8)
				throw (0);
			SoundObj result(sampleRate, 1, 8, dataSize / numChannels);
